...
function getPassword(values: any): void {
  ...
  if (values.numCharacters < 8 || values.numCharacters > 16)
    return console.log(
      "Favor de seleccionar un valor entre 8 y 16, no aceptamos " +
        values.numCharacters
    );
  let count = 0,
    password = "";
  ...
  console.log(values.numCharacters + ":     " + password);
}
...
function generarRandomBoolean(): boolean {
...
for (let X = 0; X < 10; X++) {
  ...
  getPassword({
    numCharacters: randomGenerator(1, 20),
    boolLowerCase: generarRandomBoolean(),
    boolNumbers: generarRandomBoolean(),
    boolSimbols: generarRandomBoolean(),
  });
}
